...
//	DataPacket() : packet(), packetSize(sizeof(T)){		}
	DataPacket(PACKET_TYPE_ENUMERATOR type) : packet(), packetSize(sizeof(T)), type(type) {	}
...
{
public:
	DataPacketParse() : headerData((PACKET_TYPE_ENUMERATOR)0),
		receiveIndex(0),
		headerStartIndex(0)
	{
	...
	void readPacketByte(unsigned char c) {
		...
		#ifdef PARSE_DEBUG
			serialWrite("Read[");
			serialWriteInt(receiveIndex);			
			serialWrite("] = ");
			serialWriteInt(int(receiveBuffer[receiveIndex]));						
			serialWriteLn("");
		#endif
		...
		case WAITING_HEADER_START:
			...
			#ifdef PARSE_DEBUG
				serialWriteLn("_checkHeaderStart");
			#endif
			...
			if (_checkHeaderStart()) {
				...
				#ifdef PARSE_DEBUG
					serialWriteLn("set state WAITING_HEADER_END");
				#endif		
		...
		case WAITING_HEADER_END:
			if (_checkHeaderEnd()) {
				...
				#ifdef PARSE_DEBUG
					serialWriteLn("set state RECEIVING_DATA");
				#endif				
		...
		case RECEIVING_DATA:
			if (_recvData()) {
				...
				#ifdef PARSE_DEBUG
					serialWriteLn("set state CHECK_DATA_INTEGRITY");
				#endif
		...
		case CHECK_DATA_INTEGRITY:
			...
			#ifdef PARSE_DEBUG
				serialWrite("CRC rec:");
				serialWriteInt(receiveBuffer[receiveIndex]);
				serialWrite(" calc:");
				serialWriteInt((int)crc);	
				serialWrite(" payl sz:");
				serialWriteInt((int)headerData.packet.data.payloadSize);				
				serialWriteLn("");			
			#endif	
